...
| **K** | 从一般到抽象 | 这个领域一般怎么理解？ | 广泛认知→归纳→核心思想。每个定义用 `::: definition` 块 + 展开说明 + 物理/几何意义 + 具体例子。禁止纯表格无叙述 |
...
| **K** | `# K：{标题}` | 定义用 `::: definition` + 展开说明 + 物理意义 + 例子 |
...
6. **对比强化**——用表格对比不同方法/概念
7. **Remark 穿插**——每 2-3 段就有一个 `::: remark` 展开旁注（历史背景、物理类比、数学注记）
...
**K — 从一般到抽象**
Core definitions and facts. Each definition gets a `::: definition` block followed by **expanded explanation** in natural prose. For each definition, provide:
...
- A `::: remark` block for digressions or additional context
...
1. State the theorem in a `::: theorem` block
2. Provide a **full proof** in a `::: proof` block — do not skip steps
...
| 4 | **K：从一般到抽象** | `::: definition` + 展开说明 + 物理/几何含义 + 具体例子 | 纯表格无叙述 / 定义无解释 |
...
- [ ] `::: definition`, `::: theorem`, `::: proof`, `::: remark` blocks used appropriately
